...
lines(density(ars243(n = 1e4, h = h, k = 4)), lty=2, col='blue')
lines(density(ars243(n = 1e4, h = h, k = 300)), lty=2, col='red')
curve(dexp(x, 1), from = 0, to = 2)
h <- function(x){
  return(log(dexp(x, 1)))
}
lines(density(ars243(n = 1e4, h = h, k = 4, domain = c(0,5))), lty=2, col='blue')
lines(density(ars243(n = 1e4, h = h, k = 300, domain = c(0,5))), lty=2, col='red')
curve(dunif(x), from = -0.1, to = 1.1)
h <- function(x){
  return(log(dunif(x)))
}
lines(density(ars243(n = 1e4, h = h, k = 4, domain = c(0,1))), lty=2, col='blue')
...
exp_h <- function(x) h_before_truncation(x)/integrate(h_before_truncation,-1,1)$value
h = function(x) log(exp_h(x))
curve(exp_h, from = -1, to = 1) 
curve(h, from = -1, to = 1) 
lines(density(ars243(n = 1e3, h = h, k = 3, domain = c(-1,1))), lty=2, col='blue')
